// Testes de useCurrentCompanyAssignableSellers (M1-E, E4-A1).
...
import {
  ...
  type UseCurrentCompanyAssignableSellersOptions,
} from '@/lib/hooks/useCurrentCompanyAssignableSellers';
...
function baseOptions(
  overrides: Partial<UseCurrentCompanyAssignableSellersOptions> = {},
): UseCurrentCompanyAssignableSellersOptions {
  return {
    userId: 'user-1',
    companyId: 'company-a',
...
describe('useCurrentCompanyAssignableSellers — gating', () => {
  it('Manager com membership ativa monta a query', async () => {
    ...
    const { result } = renderHook(() => useCurrentCompanyAssignableSellers(baseOptions()), { wrapper });
  ...
  it('Seller com membership ativa monta a query', async () => {
    ...
    const { result } = renderHook(
      () => useCurrentCompanyAssignableSellers(baseOptions({ membershipRole: 'seller' })),
      { wrapper },
    );
    await waitFor(() => expect(result.current.hasData).toBe(true));
    expect(result.current.queryEnabled).toBe(true);
  ...
  it('Super Admin (membershipRole null/ausente) nunca monta a query', () => {
    const { wrapper } = createWrapper();
    const { result } = renderHook(
      () => useCurrentCompanyAssignableSellers(baseOptions({ membershipRole: null })),
      { wrapper },
    );
  ...
  it('sem membership ativa (companyId ausente) não monta a query', () => {
    const { wrapper } = createWrapper();
    const { result } = renderHook(
      () => useCurrentCompanyAssignableSellers(baseOptions({ companyId: null })),
      { wrapper },
    );
    expect(result.current.queryEnabled).toBe(false);
    expect(mocks.rpc).not.toHaveBeenCalled();
  ...
  it('usuário inativo não monta a query', () => {
    const { wrapper } = createWrapper();
    const { result } = renderHook(
      () => useCurrentCompanyAssignableSellers(baseOptions({ userIsActive: false })),
      { wrapper },
    );
    expect(result.current.queryEnabled).toBe(false);
    expect(mocks.rpc).not.toHaveBeenCalled();
  ...
  it('sem userId não monta a query', () => {
    const { wrapper } = createWrapper();
    const { result } = renderHook(
      () => useCurrentCompanyAssignableSellers(baseOptions({ userId: null })),
      { wrapper },
    );
    expect(result.current.queryEnabled).toBe(false);
    expect(mocks.rpc).not.toHaveBeenCalled();
...
describe('useCurrentCompanyAssignableSellers — chave de cache e isolamento', () => {
  it('queryKey inclui companyId e identityKey (userId), namespace "assignable" distinto de "remote" (seller labels)', async () => {
    mocks.rpc.mockResolvedValue({ data: [], error: null });
    const { wrapper } = createWrapper();
    const { result } = renderHook(
      () => useCurrentCompanyAssignableSellers(baseOptions({ userId: 'user-9', companyId: 'company-z' })),
      { wrapper },
    );
    await waitFor(() => expect(result.current.isLoading).toBe(false));
    expect(result.current.queryKey).toEqual(['company', 'company-z', 'seller-labels', 'assignable', 'user-9']);
  });
  ...
  it('dois Sellers da mesma empresa não compartilham cache (chaves distintas por userId)', async () => {
    ...
    const { result: seller1 } = renderHook(
      () => useCurrentCompanyAssignableSellers(baseOptions({ membershipRole: 'seller', userId: 'seller-1' })),
      { wrapper },
    );
    const { result: seller2 } = renderHook(
      () => useCurrentCompanyAssignableSellers(baseOptions({ membershipRole: 'seller', userId: 'seller-2' })),
      { wrapper },
    );
  ...
  it('troca de empresa (A → B) não reaproveita cache: nova chave, nova chamada', async () => {
    ...
    const { result, rerender } = renderHook(
      (opts: UseCurrentCompanyAssignableSellersOptions) => useCurrentCompanyAssignableSellers(opts),
  ...
  it('logout (userId null) desmonta a query — sem reaproveitar a última chave real', () => {
    const { wrapper } = createWrapper();
    const { result, rerender } = renderHook(
      (opts: UseCurrentCompanyAssignableSellersOptions) => useCurrentCompanyAssignableSellers(opts),
      { wrapper, initialProps: baseOptions() },
    );
    rerender(baseOptions({ userId: null }));
    expect(result.current.queryEnabled).toBe(false);
  });
});

describe('useCurrentCompanyAssignableSellers — estados e sellersById', () => {
  it('sucesso: assignableSellers e sellersById refletem o retorno real (sem placeholder)', async () => {
  ...
  it('lista vazia ⇒ isEmpty true, hasData false, sellersById vazio (ex.: Manager sem nenhum Seller ativo)', async () => {
    mocks.rpc.mockResolvedValue({ data: [], error: null });
    const { wrapper } = createWrapper();
    const { result } = renderHook(() => useCurrentCompanyAssignableSellers(baseOptions()), { wrapper });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('erro do Supabase é exposto, nunca vira lista vazia silenciosa', async () => {
    ...
    expect(result.current.assignableSellers).toEqual([]);
    expect(result.current.sellersById).toEqual({});
  });
});
