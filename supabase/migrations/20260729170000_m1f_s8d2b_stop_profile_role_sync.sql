...
-- update_membership_role (docs/M1-F-SUPER-ADMIN-USER-LIFECYCLE-DESIGN.md
-- §44). Auditoria prévia (S8-D-A0/S8-D1/S8-D2-A) já confirmou zero
-- consumidor runtime de profiles.role no frontend; auditoria desta etapa
-- confirmou, no estado ativo do banco, que update_membership_role era o
-- ÚNICO escritor (UPDATE) da coluna, accept_invite só faz o INSERT
-- inicial (fora de escopo — tratado junto da remoção física da coluna no
-- S8-E), e zero policy/trigger/outra RPC lê a coluna. Os helpers legados
-- M1-C (current_profile_role, is_manager_or_admin, current_profile_
-- seller_id, current_profile_company_id) continuam existindo, sem
-- nenhum consumidor ativo — não tocados, reservados ao S8-E.
--
-- A partir desta migration, company_memberships.role muda normalmente e
-- profiles.role NUNCA MAIS é escrita por esta RPC — a coluna fica
-- congelada no último valor que já tinha (accept_invite ou uma troca de
-- role anterior a esta migration) e pode divergir da membership por
-- design; essa divergência não concede nem retira autoridade alguma (o
-- frontend nunca leu esse campo desde o S8-D2-A).
--
-- Mesma assinatura, mesmos parâmetros/defaults, mesmo retorno, mesmo
-- SECURITY DEFINER/search_path=''/grants, mesmas validações/locks/erros/
-- guarda do último Manager/ciclo de ativação-desativação de sellers —
-- nada disso muda. Única remoção real: as duas linhas
-- `update public.profiles set role = ...` (uma em cada branch) e as
-- variáveis que só existiam para decidir/registrar esse UPDATE.
--
-- ── Correção de idempotência (decisão humana explícita desta etapa) ────
-- v_needs_profile_update/v_expected_profile_role não podem simplesmente
-- desaparecer do meio do código sem mais nada: elas também alimentavam
-- (a) o curto-circuito de idempotência e (b) after_data.profile_role do
-- audit_log. Deixá-las "mortas" faria a função NUNCA MAIS se considerar
-- idempotente depois da primeira troca real (profiles.role fica
-- congelado para sempre, então "v_target_profile.role is distinct from
-- v_expected_profile_role" seria true para sempre) — toda chamada
-- repetida com o mesmo p_role passaria a gravar um audit_log novo em vez
-- de retornar cedo sem escrever nada, e o audit_log passaria a afirmar
-- (via after_data.profile_role = v_expected_profile_role) que o profile
-- foi atualizado para um valor que nunca chega a ser escrito. As duas
-- variáveis são removidas por completo: a decisão de idempotência
-- depende só de mudanças reais ainda executadas pela função (membership/
-- seller); before_data/after_data.profile_role passam a mostrar sempre
-- v_target_profile.role (o valor real, nunca tocado por esta RPC — igual
-- antes e depois, honesto sobre o que de fato aconteceu).
begin;
...
begin
  ...
  -- M1-F S8-D2-B: profiles.role não é mais lida/escrita aqui — a
  -- idempotência depende só de efeitos reais ainda executados por esta
  -- função (membership/seller).
  ...
  if not v_needs_membership_update and not v_needs_seller_update then
    ...
    -- profiles.role divergente da membership NUNCA conta como trabalho
    -- pendente — é o estado esperado por design a partir desta etapa.
    return query select v_membership.id, v_membership.profile_id, v_membership.company_id, v_membership.role;
    return;
  end if;

  -- profile_role reflete o valor REAL de profiles.role, sempre — esta
  -- RPC não o altera mais, então before/after são iguais nesse campo,
  -- honestos sobre o que de fato aconteceu.
